...
def _load_golden(category: str | None = None) -> list[dict[str, Any]]:
    ...
    path = Path(__file__).parent.parent.parent / "fixtures" / "langgraph_react" / "golden_queries.yaml"
    with open(path) as f:
...
# The dummy_web_search tool always returns content containing these terms.
# If the response includes them, the tool was called and its output incorporated.
...
async def test_tool_selection_accuracy(
    ...
    # Primary: response should incorporate search tool output
...
async def test_tool_not_called_for_greeting(run_eval: Any) -> None:
    ...
    # Even without tool_calls, verify response is a greeting, not a search result
    text_lower = result.response.lower()
