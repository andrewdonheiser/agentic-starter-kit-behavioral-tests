...
class AgenticEvalAdapter(FrameworkAdapter):
    ...
    async def _run_async(
        ...
        try:
            ...
            _report_fatal(callbacks, str(e))
            raise

        scorer_names = resolve_scorers(benchmark)

        # Phase 2: LOADING_DATA (10%)
        _report(
        ...
        if not queries:
            _report_fatal(callbacks, "No queries found in benchmark")
            raise ValueError("No queries found in benchmark")

        # Phase 3: RUNNING_EVALUATION (30-80%)
        _report(
        ...
        async with httpx.AsyncClient(
            verify=False, timeout=httpx.Timeout(params.timeout_seconds)
        ) as client:
            for i, query_spec in enumerate(queries):
                ...
                try:
                    task_config = job_spec_to_task_config(
                        ...
                        expected_tools=query_spec.expected_tools or None,
                        params=params,
        ...
        # Phase 5: PERSISTING_ARTIFACTS (90%)
        _report(
        ...
        if params.mlflow_tracking_uri:
            _log_mlflow_run(
                params.mlflow_tracking_uri,
                params.mlflow_experiment_name or config.benchmark_id,
                config, eval_results, overall_score, duration, len(queries),
            )

        # Phase 6: Build and report results
...
def _score_result(
    ...
    """Run the requested scorers against a single eval result."""
    scores: list[Score] = []
...
def _score_to_eval_result(score: Score, num_samples: int) -> EvaluationResult:
    """Map our Score dataclass to EvalHub's EvaluationResult."""
...
def _report_fatal(callbacks: JobCallbacks, error_message: str) -> None:
